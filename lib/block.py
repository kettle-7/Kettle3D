...
# Please note that xpos, ypos and zpos are positions within the chunk, if you want the position within the
# world, use absx, absy and absz.
...
class concrete(Block):
	def __init__(self, chunk, xpos, ypos, zpos, renderer): # blockpath is expected to be provided within this class. Extra parameters are allowed such as blockstates.
		...
		self.ypos = ypos
		self.zpos = zpos
		self.blockpath = Filename.fromOsSpecific(launcherbase.directory).getFullpath() + 'assets/concrete'
