// This script is attached to all the blocks with prefab models
...
public class PrefabBlock : MonoBehaviour
{
    ...
    void Start()
    {
        this.SelfModel = gameObject; // Set the SelfModel field to the GameObject of the Parent of the 
    ...
    void OnMouseOver() { // This function gets called when the mouse is on top of this side of the block.
        ...
        if(Input.GetMouseButtonDown(0)) { // If the user is left-clicking, Destroy the block.
            Destroy (gameObject); // We want to destroy the whole block, not just this side of it. transform is the position of this object in the block, parent is the transform of the whole block and gameObject is the GameObject that a tranform (or script) is attached to.
            game.worldmap.Remove(gameObject); // And then we want to remove it from worldmap, so that we don't get issues later on.
        }

        if (Input.GetMouseButtonDown(3)) {
            game.PickedBlock = gameObject;
        }
    }
}
